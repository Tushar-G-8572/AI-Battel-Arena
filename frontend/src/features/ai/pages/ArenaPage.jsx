...
const SkeletonCard = ({ title }) => (
  <div className="flex flex-col bg-[#031d4b] rounded-xl border border-[#2b4680]/25 overflow-hidden h-full animate-pulse">
    <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-[#00225a] flex items-center gap-2.5">
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-12 sm:py-20 gap-4 text-center px-4">
    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-[#00225a] rounded-2xl flex items-center justify-center">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#5b74b1"
    ...
    <div>
      <p className="text-[#dee5ff] font-semibold text-sm sm:text-base">No battle yet</p>
      <p className="text-[#5b74b1] text-xs sm:text-sm mt-1 max-w-xs">
...
const ArenaPage = () => {
  ...
  useEffect(() => {
    if (prevLoadingRef.current && !loading) {
      if (aiResponse) toast.success("Battle completed");
    }
  ...
  return (
    <div className="min-h-screen w-full bg-[#060e20] flex flex-col">
      ...
      <div className="flex w-full flex-1 overflow-hidden">
        <SideBar />
        <main className="flex-1 mt-2 min-w-0 px-3 sm:px-6 py-4 sm:py-6 flex flex-col gap-4 sm:gap-6 overflow-y-auto">
          {/* Page header */}
          <div className="flex flex-col gap-1">
            <h1 className="text-[#dee5ff] ml-15 text-lg sm:text-xl font-bold tracking-tight">
              Battle Arena
            </h1>
            <p className="text-[#91aaeb] text-xs sm:text-sm">
          ...
          {error && (
            <div id="arena-error" className="px-4 py-3 bg-[#7f2927]/30 border border-[#7f2927]/50 rounded-lg text-[#ff9993] text-xs sm:text-sm">
              {error}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-4 sm:gap-6">
              <div className="bg-[#06122d] border border-[#2b4680]/30 rounded-xl px-4 sm:px-5 py-3 sm:py-3.5">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-[11px] uppercase tracking-widest text-[#91aaeb] font-medium">Problem</p>
                  <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 bg-[#00225a] text-[#5b74b1] rounded font-medium">
                    AI Battle Arena
                  </span>
                </div>
                <p className="text-[#dee5ff] text-xs sm:text-sm font-medium leading-relaxed">
                  {aiResponse?.problem}
                </p>
              </div>
