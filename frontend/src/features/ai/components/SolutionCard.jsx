...
const ScoreBadge = ({ score }) => {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded border text-xs font-bold flex-shrink-0 ${getColor()}`}>
...
const SolutionCard = ({ title, content, score, reasoning, modelLabel }) => {
  return (
    <div className="flex flex-col bg-[#031d4b] rounded-xl border border-[#2b4680]/25 overflow-hidden h-full">
      {/* Card Header */}
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 bg-[#00225a] gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-2 h-2 rounded-full bg-[#2563eb] flex-shrink-0" />
          <span className="text-[#dee5ff] font-semibold text-sm truncate">{title}</span>
          {modelLabel && (
            <span className="text-[10px] text-[#91aaeb] uppercase tracking-widest hidden sm:inline truncate">
              · {modelLabel}
            </span>
          )}
        </div>
      ...
      {/* Solution Content */}
      <div className="flex-1 px-4 sm:px-5 py-3 sm:py-4 overflow-y-auto max-h-[320px] sm:max-h-[420px] custom-scroll">
        {content ? (
      ...
      {/* Reasoning footer */}
      {reasoning && (
        <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-[#05183c] border-t border-[#2b4680]/20">
